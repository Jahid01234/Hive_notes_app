...
//
//
//   final RxList<NoteModel> pinnedNotes = <NoteModel>[].obs;
//   final RxList<NoteModel> otherNotes = <NoteModel>[].obs;
//
//   @override
//   void onInit() {
//     super.onInit();
...
class HomeController extends GetxController {
  ...
  void applyFilterAndSort() {
    ...
    result = _sortList(result);

    notes.value = result;

    if (currentFilter.value == FilterType.all) {
      pinnedNotes.value = result.where((n) => n.isPinned).toList();
      otherNotes.value = result.where((n) => !n.isPinned).toList();
    } else {
      pinnedNotes.value = [];
      otherNotes.value = result;
    }
  }

  List<NoteModel> _sortList(List<NoteModel> list) {
    switch (currentSort.value) {
      case SortType.newest:
  ...
  int get pinnedCount => allNotes.where((n) => n.isPinned).length;
  int get favouriteCount => allNotes.where((n) => n.isFavourite).length;
  bool get shouldShowSections => currentFilter.value == FilterType.all && pinnedNotes.isNotEmpty;
}